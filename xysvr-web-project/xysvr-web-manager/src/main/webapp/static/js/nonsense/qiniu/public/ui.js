...
function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        $('#content').val("nonsense/" + file.name);
        $('#content').prop({
            readonly : true
        });
...
FileProgress.prototype.setChunkProgess = function(chunk_size) {
    ...
    for (var i = 1; i <= chunk_amount; i++) {
        ...
        var progressBar = $("<div/>");
        progressBar.addClass("progress-bar progress-bar-info text-left").attr('role', 'progressbar').attr('aria-valuemax', 100).attr('aria-valuenow', 0).attr('aria-valuein', 0).width('0%').attr('id',
                this.file.id + '_' + i).text('');
...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    var showImg = $('<img src="' + ctxPath + '/static/js/qiniu/loading.gif"/>');

    progressNameTd.append(Wrapper);

    if (!isImg) {
        showImg.attr('src', ctxPath + '/static/js/qiniu/default.png');
        ...
        $(img).on(
                ...
                function() {
                    ...
                    linkWrapper.attr('href', url).attr('title', '查看');
                    ...
                    function initImg(url, key, height) {
                        ...
                        modalBody.find('img').attr('src', ctxPath + '/static/js/qiniu/loading.gif');
                    ...
                    if (!(ie && ie <= 9)) {
                        var exif = /*Qiniu.exif(res.key)*/false;
                        if (exif) {
                        ...
                        var infoArea = $('<div/>');
                        var infoInner = '<div>格式：<span class="origin-format">' + imageInfo.format + '</span></div>' + '<div>宽度：<span class="orgin-width">' + imageInfo.width + 'px</span></div>'
